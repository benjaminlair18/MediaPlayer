...
struct Config {
    ...
    static let TB_SAFEAREA_PADDING:CGFloat = 50.0
    static let TB_LINE_WIDTH:CGFloat = 30.0
    static let TB_FONTSIZE:CGFloat = 30.0
...
class BWCircularSlider: UIControl {
    ...
    override init(frame: CGRect) {
        ...
        let textFieldRect = CGRectMake(
            ...
            (frame.size.height - fontSize.height) / 2.75,
    ...
    override func drawRect(rect: CGRect){
        ...
        CGContextAddArc(ctx, CGFloat(self.frame.size.width / 2.0), CGFloat(self.frame.size.height / 2.0), radius, 0, CGFloat(M_PI ), 1)
        UIColor(red: 0.2, green: 0.1, blue: 0.3, alpha: 1.0).set()
        ...
        CGContextAddArc(imageCtx, CGFloat(self.frame.size.width/2)  , CGFloat(self.frame.size.height/2), radius, 3.14-0.180, CGFloat(DegreesToRadians(Double(angle))) , 1);
        UIColor.redColor().set()
        
        //Use shadow to create the Blur effect
        CGContextSetShadowWithColor(imageCtx, CGSizeMake(0, 0), CGFloat(self.angle/12), UIColor.blackColor().CGColor);
    ...
    func moveHandle(lastPoint:CGPoint){
        ...
        if( angle > 9 && angle < 171)
        {
        textField!.text = "\((170 - angle) * 10 / 16)"
            
            rangle = Int((170 - angle) * 10 / 16)
        }
        else if( angle < 10 || angle > 358)
        {
            textField!.text = "100"
              rangle = 100
